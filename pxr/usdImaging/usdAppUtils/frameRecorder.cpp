        ...
        const std::string& outputAOV)
{
    ...
    const auto writeOneFrame = [&mtx, &successAllFrames](TextureBufferWriter *textureBufferWriter,
        const std::string& outputImagePath) -> void {
        const bool success = textureBufferWriter->WriteFromTemporaryStorage(outputImagePath);
        mtx.lock();
        successAllFrames &= success;
        mtx.unlock();
    };
    ...
    for (int i = 0; i < numTimeCodes; ++i) {
        ...
        // If rendering only '1' frame - just use the 'main' thread, else use multi-threading.
        if (numTimeCodes == 1 && i == 0) {
          TextureBufferWriter writer(&_imagingEngine);
          successAllFrames = writer.WriteFromTemporaryStorage(thisOutputImagePath);
        } else {
          // Copy data from GPU to CPU.
          // Writers are created on heap. We need to keep them until thread finishes writing to disk.
          writers.emplace_back(new TextureBufferWriter(&_imagingEngine));

          // Write data to disk (via threads).
          threads.emplace_back(std::thread(writeOneFrame, writers.back(), thisOutputImagePath));
        }
